 ...
 * update_database.php
 *
 * Applies the full set of schema updates required for the SNMP/fiber/patch-panel work.
 * - Run from the web or CLI.
 * - Makes idempotent checks before applying each change.
 * - Reports success/failure per step in an HTML table.
 *
 * IMPORTANT:
 * - Make a full DB backup before running on production.
 * - This script runs many DDL statements; run on a maintenance window if possible.
 */
